...
firePallet = {  0 : (7,7,7), 1 : (31,7,7), 2 : (47,15,7), 3 : (71,15,7), 4 : (87,23,7), 5 : (103,31,7), 6 : (119,31,7), 
...
class FireEffect(object):
    ...
    def fire_calculate(self,layer=1):
        for coord in self.canvas[layer].keys():
            ...
            if self.canvas[layerDown].get((coord[0],yDown))[1] > 0:
                ...
                if coordNeighbor in self.canvas[layer].keys() and decay == 1:
                    self.canvas[layer][(coordNeighbor)][1] = self.canvas[layerDown].get((coord[0],yDown))[1] - random.randint(0, 1)
                else:    
                    self.canvas[layer][(coord)][1] = self.canvas[layerDown].get((coord[0],yDown))[1] - random.randint(0, 1)
    ...
    def fire_intensity(self,intensity):
        for coord in self.canvas[0].keys():
            self.canvas[0].get(coord)[1] = intensity
